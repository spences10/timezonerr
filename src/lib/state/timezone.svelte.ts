...
let current_times = $state<Array<TimezoneConfig & { time: string; date: string }>>([]);
...
export function add_timezone(timezone: TimezoneConfig) {
    ...
    if (typeof window !== 'undefined') {
        localStorage.setItem('selected_timezones', JSON.stringify(timezones));
        update_times();
    }
}

export function remove_timezone(timezone_to_remove: string) {
    timezones = timezones.filter((tz: TimezoneConfig) => tz.timezone !== timezone_to_remove);
    if (typeof window !== 'undefined') {
        localStorage.setItem('selected_timezones', JSON.stringify(timezones));
        update_times();
    }
